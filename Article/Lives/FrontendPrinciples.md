...
# 写给自学前端同学的编程法则 {#frontend-principles}
...
## 2. 法则二：三步走 {#three-step-rule}

写代码要遵循这个步骤：

### 第一步：让它能跑（Make it work） {#make-it-work}

别管代码多丑，先让功能正常运行；别担心性能问题，先确保逻辑正确；别想架构问题，先把基本功能实现。

### 第二步：让它正确（Make it right） {#make-it-right}

补充测试用例，确保各种边缘情况都能处理。

### 第三步：让它快速（Make it fast） {#make-it-fast}

进行性能优化。

很多时候，你会发现根本走不到第三步。 因为大部分场景下，“够用”就行了。

## 3. 法则三：每个函数只做一件事 {#single-responsibility}
...
## 5. 法则五：保持一个抽象层次 {#single-abstraction-level}
